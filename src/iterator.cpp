...
public:
  ...
  // prefix increment will increment the value before it is used
  // if int a = 5, then b=++a; a=6 and b=6 because a is incremented first, then
  // assigned to b
  // because we will first iteratore then use the resulting value in the
  // assignment here we go next and then return the node after it has gone to
  // the next node so that this will be used in the following expression
  ...
  // the increment.
  //
  // postfix increment will use the current value in an expression, and then
  // increment it after
  // if a = 5, then b=a++; b=5 and then a=6 because a is incremented after
  // assignment to b
  //
  // here we advance the pointer, but return the original value. This way,
  // we can use the original value in an expression, but the pointer has been
  // advanced so if we use it again, it is on the next node
